...
across the globe. This dataset contains "noise". The [SWIFT](./provided-resources.md#swift) section has more details.
...
returns data for a number of records at a time therefore it needs to be pulled repeatedly.
- `/transactions/swift` provides `SWIFT` transactions for accounts _**globally**_. This endpoint returns data for
a number of records (default=5000) at a time therefore it needs to be pulled repeatedly.
- `/exchange-rates` provides currencies' exchange rates for `SWIFT` transactions for accounts _**globally**_.

The `/transactions/swift` endpoint contains some noisy-data and invalid payments. 
We leave this up to the candidate to decide what to do with these invalid data.

Please keep in mind, both `/transactions/sepa` and `/transactions/swift` require pagination, by providing:
- _limit_ (max=5000) - response limit
- _after-uuid_ - last uuid from the previous response
- _after-timesamp_ - last timestamp from the previous response
